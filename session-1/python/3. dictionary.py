...
new_dict = {'name': 'John', 'roll': 30, 'grades': 'A'}
# Updating dictionary
new_dict['subjects'] = "english" 
new_dict['marks'] = 78
print(new_dict)
removed_val = new_dict.pop('marks') 
print(removed_val)
new_dict.update({'age': 13, 'email': 'john@example.com'})  # Updates the dictionary
...
students_sorted_by_grade = sorted(students, key=lambda x: x['grade'], reverse=True)#reverse dye cuto theke boro sort
...
for student, average_grade in average_grades.items():
    print(f"{student}: {average_grade:.2f}")#Inside the f-string, you can embed Python expressions by placing them within curly braces {}
